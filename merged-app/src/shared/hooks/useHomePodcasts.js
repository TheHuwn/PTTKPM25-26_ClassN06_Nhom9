...
/**
 * Custom hook to fetch and manage podcasts data for home page
 */
export const useHomePodcasts = () => {
  ...
  const fetchPodcasts = async () => {
    try {
      ...
      setPodcasts(allPodcasts.slice(0, 3).map(transformPodcast));

      console.log("[useHomePodcasts] Data loaded successfully");
    } catch (err) {
      setError(err.message);
      console.error("[useHomePodcasts] Error:", err);
    } finally {
      setLoading(false);
    }
  };
...
export default useHomePodcasts;
